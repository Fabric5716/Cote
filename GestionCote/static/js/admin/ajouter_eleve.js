// static/js/admin/ajouter_eleve.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('date_inscription').addEventListener('change', generateMatriculePreview);
});
    ...
    function next() {
        if (validateStep(currentStep)) {
            if (currentStep < totalSteps) {
                ...
                document.getElementById('eleveForm').dispatchEvent(new Event('submit'));
            }
        }
    }
...
function validateStep(step) {
    ...
    if (step === 3) {
        ...
        if (classeSelect && classeSelect.options.length <= 1) {
            Swal.fire('Erreur', 'Aucune classe disponible. Veuillez d\'abord créer des classes dans "Gestion des classes et options".', 'error');
            return false;
        }
        if (!classeValue) {
...
// Génération de l'aperçu du matricule
function generateMatriculePreview() {
    ...
    const compteurProvisoire = '001';
    const matriculePreview = `KABA${annee}${jour}${mois}${compteurProvisoire}`;
    matriculeField.value = matriculePreview;
}
        ...
        .then(data => {
            if (data.success) {
                ...
                if (submitBtn) {
                    ...
                    submitBtn.innerHTML = 'Confirmer et inscrire';
                }
            }
        })
        .catch(error => {
